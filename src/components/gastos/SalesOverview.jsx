...
export default function SalesOverview() {
  ...
  return (
    <div className="bg-white/15 backdrop-blur-sm border border-gray-300 shadow-lg p-4 rounded-lg ml-4 h-[500px] rounded-lg shadow p-6 flex flex-col flex-1">
      <h1 className="text-4xl font-semibold text-white">Gastos</h1>
      <div className="flex flex-col lg:flex-row">
        <div className="w-full lg:w-3/4 h-80 ">
          <Doughnut data={data} options={options} plugins={[centerTextPlugin]} />
        </div>
        <div className="w-full lg:w-1/4 flex flex-col justify-ler">
          <p className="text-4xl font-semibold text-white mt-4">$56,734.36</p>
          <p className="text-green-500 text-2xl mt-2">+12.76%</p>
          <ul className="mt-16">
            {expenses.map((expense, index) => (
              <li key={index} className="flex items-center m-5">
                <span
